...
class Catalogue extends BaseController{
	...
	public function index($page = 1){
		...
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/file/catalogue/index'
		]);
		if($flag == false){
		...
		$config['total_rows'] = $this->AutoloadModel->_get_where([
			...
			'join' =>  [
				[
					'file_translate as tb2','tb1.id = tb2.objectid AND tb2.module = \''.$this->data['module'].'\'   AND tb2.language = \''.$this->currentLanguage().'\' ','inner'
				],
			],
			'keyword' => $keyword,
			'where' => $where,
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/file/catalogue/index','perpage' => $perpage], $config);
			...
			$this->data['fileCatalogueList'] = $this->AutoloadModel->_get_where([
		...
		$this->data['template'] = 'backend/file/catalogue/index';
	...
	public function create(){
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/file/catalogue/create'
		]);
		if($flag == false){
 			$session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
 			return redirect()->to(BASE_URL.'backend/dashboard/dashboard/index');
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($resultid > 0){
					...
					$this->nestedsetbie->Action();
 					...
 					return redirect()->to(BASE_URL.'backend/file/catalogue/create');
		 		}
		...
		$this->data['template'] = 'backend/file/catalogue/create';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function update($id = 0){
		...
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/file/catalogue/update'
		]);
		if($flag == false){
 			$session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
 			return redirect()->to(BASE_URL.'backend/dashboard/dashboard/index');
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id, tb2.title, tb2.canonical, tb2.description, tb2.content, tb2.meta_title, tb2.meta_description, tb1.parentid, tb1.image, tb1.album, tb1.publish, tb2.router, tb2.icon',
			'table' => $this->data['module'].' as tb1',
			'join' =>  [
					[
						'file_translate as tb2','tb1.id = tb2.objectid AND tb2.module = \''.$this->data['module'].'\'   AND tb2.language = \''.$this->currentLanguage().'\'  ','inner'
		...
		if(!isset($this->data[$this->data['module']]) || is_array($this->data[$this->data['module']]) == false || count($this->data[$this->data['module']]) == 0){
			$session->setFlashdata('message-danger', 'Nhóm Bài Viết không tồn tại');
 			return redirect()->to(BASE_URL.'backend/file/catalogue/index');
		}
		...
		if($this->request->getMethod() == 'post'){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($flag > 0){
		 			$flag = $this->AutoloadModel->_update([
			 			'table' => 'file_translate',
			 			'where' => ['objectid' => $id,'module' => 'file_catalogue', 'language' => $this->currentLanguage()],
			 			'data' => $updateLanguage
			 		]);

			 		if($updateLanguage['canonical'] != $this->data[$this->data['module']]['canonical'] || $updateLanguage['router'] != $this->data[$this->data['module']]['router']){
			 			insert_router([
			 				'method' => 'update',
			 				'id' => $id,
			 				'language' => $this->currentLanguage(), 
		...
		$this->data['template'] = 'backend/file/catalogue/update';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function delete($id = 0){
		...
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/file/catalogue/delete'
		]);
		if($flag == false){
 			$session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
 			return redirect()->to(BASE_URL.'backend/dashboard/dashboard/index');
		...
		$this->data['template'] = 'backend/file/catalogue/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	private function detect_language(){
		...
		if(isset($languageList) && is_array($languageList) && count($languageList)){
			foreach($languageList as $key => $val){
				$select = $select.'(SELECT COUNT(objectid) FROM file_translate WHERE file_translate.objectid = tb1.id AND file_translate.language = "'.$val['canonical'].'") as '.$val['canonical'].'_detect, ';
				$i++;
			}	
		}
